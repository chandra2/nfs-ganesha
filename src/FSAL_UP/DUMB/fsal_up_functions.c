...
fsal_status_t dumb_fsal_up_fd_close(fsal_up_event_data_t * pevdata)
{
  cache_inode_status_t cache_status;
  cache_entry_t *pentry = NULL;
  fsal_attrib_list_t attr;
  ...
  LogDebug(COMPONENT_FSAL_UP,
          "FSAL_UP_DUMB: found entry %p type %u",
          pentry, pentry->internal_md.type);

  /* Lock the entry */
  P_w(&pentry->lock);
  LogDebug(COMPONENT_FSAL_UP, "FSAL_UP_DUMB: calling cache_inode_close");
  cache_inode_close(pentry, NULL, &cache_status);
  V_w(&pentry->lock);
...
fsal_up_event_functions_t dumb_event_func = {
  ...
  .fsal_up_fd_close = dumb_fsal_up_fd_close
};
