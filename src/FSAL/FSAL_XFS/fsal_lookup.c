  ...
  if(!p_parent_directory_handle)
    {
      ...
      if(p_object_attributes)
        {
          status = XFSFSAL_getattrs((fsal_handle_t *)p_object_handle, (fsal_op_context_t *)p_context, p_object_attributes);
    ...
    )
{
  ...
  status = fsal_internal_Path2Handle((xfsfsal_op_context_t *)p_context, p_path, (xfsfsal_handle_t *)object_handle);
  if(FSAL_IS_ERROR(status))
